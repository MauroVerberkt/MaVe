...
The static/manual discoverability layer is already in place:

| File | Location | Purpose |
|------|----------|---------|
| `llms.txt` | `docs/static/llms.txt` → `/MaVe/llms.txt` | AI-readable project description: canonical URL, packages, key doc links |
| `robots.txt` | `docs/static/robots.txt` → `/MaVe/robots.txt` | Crawler directives and sitemap reference |
| `sitemap.xml` | Generated by Docusaurus at build time | Machine-readable page index for search engines |
| `.opencode/instructions.md` | Repo root | Agent instructions: layout, conventions, workflow |

These are hand-maintained and cover the "what is this repo?" question at a static level.
PROP-021 builds the dynamic/generated layer on top.

## Vision

A repository that any agent can pick up and immediately understand:

- **"What is this repo?"** → answered by `llms.txt`, `llms-full.txt`,
  `.opencode/instructions.md`, and `.ai/manifest.json`
...
# 6. Assemble llms-full.txt from instructions.md + doc intros + public API summaries
```
